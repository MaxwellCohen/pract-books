import { BookOpenIcon, Building2Icon, CalendarDaysIcon, GlobeIcon, HashIcon } from '@/components/ui/icons';
...
import type { BookDetails as BookDetailsType } from '@/features/book/book-queries';
import type { ComponentChildren } from 'preact';

const DETAIL_SIZES = '(min-width: 768px) 18rem, 60vw';

export function BookDetail({ book }: { book: BookDetailsType | null }) {
    ...
    <article className="flex flex-col gap-8 md:flex-row md:gap-10">
      ...
      <div className="min-w-0 flex-1">
        ...
        <dl className="border-divider dark:border-divider-dark mt-8 grid grid-cols-1 gap-x-8 gap-y-4 border-t pt-6 sm:grid-cols-2">
          <Fact icon={<BookOpenIcon className="size-4" />} label="Pages">
            {book.num_pages ? book.num_pages.toLocaleString() : 'Unknown'}
          </Fact>
          <Fact icon={<GlobeIcon className="size-4" />} label="Language">
            {getLanguageLabel(book.language_code)}
          </Fact>
          <Fact icon={<CalendarDaysIcon className="size-4" />} label="Published">
            {book.publication_year ?? 'Unknown'}
          </Fact>
          <Fact icon={<Building2Icon className="size-4" />} label="Publisher">
            {book.publisher ?? 'Unknown'}
          </Fact>
          <Fact icon={<HashIcon className="size-4" />} label="ISBN">
...
function Fact({ children, icon, label }: { children: ComponentChildren; icon: ComponentChildren; label: string }) {
  return (
