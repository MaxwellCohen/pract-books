...
export function BookFilters({ idPrefix }: { idPrefix: string }) {
  ...
  const [draft, setDraft] = useState<SearchParams | null>(null);
  const filters = draft ?? current;
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col" data-filtering={pending ? '' : undefined}>
      <div className="min-h-0 flex-1 touch-pan-y [scrollbar-gutter:stable] overflow-x-hidden overflow-y-auto overscroll-contain px-1 pb-6">
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-muted text-xs font-semibold tracking-wide uppercase" htmlFor={`${idPrefix}-filter-language`}>
              Language
            </label>
            <Select
      ...
      {activeCount > 0 ? (
        <div className="border-divider dark:border-divider-dark border-t pt-3">
          <Button
            className="w-full"
            onClick={() => {
              const next = filters.delay ? { delay: filters.delay } : {};
              setDraft(next);
              replace(next);
            }}
            variant="secondary"
